...
#ifndef RailOSTIMETABLE_HXX
#define RailOSTIMETABLE_HXX
...
/*! @brief      RailOS Timetable Editor Timetable Class
...
class RailOSTimetable
{
    private:
        ...
        QMap<QString, RailOSService*> _services;
    ...
    public:
        /*! @brief Create a new timetable object
         ...
         @return void
        */
        RailOSTimetable(QString file_name="NewTimetable_"+QDateTime::currentDateTime().toString("HH_mm_dd-MM-yy")+".ttb") : _file_name(file_name) {}

        /*! @brief Add a new service to the timetable
        @param int_id Unique integer ID for service
        @param start_time Service start time
        @param id RailOS style service ID
        ...
        */
        ...
        {
            _services[id] = new RailOSService(int_id, start_time, id, description, start_speed, max_speed, mass, max_brake_force, power);
        }

        /*! @brief Add a new service to the timetable
        @param service An RailOSService to add
        @return void
        */
        void addService(RailOSService* service)
        {
            _services[service->getID()] = service;
        }

        /*! @brief Remove a service from the timetable
        @param service_ref RailOS service ID of service to remove
        @return void
        */
        ...
        */
        QMap<QString, RailOSService*> getServices() const {return _services;}
        ...
        */
        RailOSService* operator[](const QString&);

        /*! @brief Access services from the timetable by index
        @return Pointer to service
        */
        RailOSService* operator[](const int&);

        /*! @brief Pointer to first timetable entry
        @return Pointer to service
        */
        RailOSService* begin() {return this->operator[](0);}

        /*! @brief Pointer to last timetable entry
        @return Pointer to service
        */
        RailOSService* end() {return this->operator[](_services.size()-1);}
